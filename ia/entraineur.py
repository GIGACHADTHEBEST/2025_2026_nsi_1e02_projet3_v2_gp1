...
IA : entraineur.py
==================
Fait s'entraîner deux agents l'un contre l'autre (self-play).
Conçu pour être appelé depuis le contrôleur dans un thread séparé.
"""
...
MAX_COUPS = 150   # limite anti-boucle par partie
...
class Entraineur:
    def __init__(self,
        ...
        """
        on_fin_partie(n_partie, resultat_dict) : callback appelé après chaque partie.
        """
        self.blanc         = agent_blanc
        self.noir          = agent_noir
        self.on_fin_partie = on_fin_partie

    def jouer_une_partie(self) -> dict:
        plateau = Plateau()
        courant = BLANC
        n_coups = 0
        ...
        while n_coups < MAX_COUPS:
            ...
            if not choix:
                ...
                break
            pion, chemin = choix
            # Le pion de la copie de l'agent peut différer du plateau réel
        ...
        return {"gagnant": gagnant, "pts_blanc": pts[BLANC],
                "pts_noir": pts[NOIR], "coups": n_coups}

    def entrainer(self, n: int):
        for i in range(n):
